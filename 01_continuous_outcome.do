...
/*I want an arbitrary number of reminders where individuals are allowed to be more/less responsive to reminders based on observables - XB_k for all K reminders. Unlike GMM, stata ml doesn't allow me to simply specify the number of equations (to my knowledge) so I will hack this by:
...
ml model lf0 ml_stacked_ordered_heckman (Linear_s2: ent_linear_observed = `controls') `Equations' /athrho /lnsigma,  missing vce(robust)
*ml init initial, copy
ml maximize

predict Y_hat_stacked_mle 

local rho = (expm1(2*/athrho))  /  (exp(2*/athrho)+1)
local sigma = exp(/lns)
di `rho'
di `sigma'
...
*Code up 2-step with linear second stage. See if you can learn from heckman command code for twostep.

*Do later if at all.



*Add loops in the code here and above to have an arbitrary number of reminders. You need to change the MLE syntax above to set the number of equations




*Code up probit 2nd stage - MLE (code margins afterward)
*Bivariate probit - allow reminder to affect differently given X
...
program ml_stacked_ordered_heckman_test
	...
	mleval `lns' = `b', eq(`=$notification_count+$notification_count+1')
	tempname rho sig lambda
	...
	*Loop over K Heckman selection models (only 1 sigma and rho)
	quietly replace `lnf' = 0 //prime likelihood
	forv k=1/2 { //these identify main rho
	...
	forv k=3/$notification_count { //these identify deviations from main rho
	quietly replace `lnf' =  `lnf'+log(1 - normal((`xbs`k'')/1)) if ${ML_y`=`k'+1'}==0   // Don't respond after kth notice
	quietly replace `lnf' =  `lnf'+lnnormalden($ML_y1, `xb_main', `sig') + lnnormal((`xbs`k'' + `rho_`k''/`sig'*($ML_y1 - `xb_main'))/sqrt(1 - `rho_`k''^2)) if ${ML_y`=`k'+1'}==1 
	}	
end
...
local athrhos ""
forv k = 3/$notification_count { //this should always be 3! The difference between groups 1 and 0 can't identify anything (0 unobserved), the difference between 1 and 2 identifies the "main rho" and the difference between 1 and 3 (the 3rd difference) identifies a new rho parameter. Subsequent notifications attempt to rationalize outcomes for group k, which identify rho_k.
	local athrhos "`athrhos' /athrho`k'"
}
di "`athrhos'"


ml model lf0 ml_stacked_ordered_heckman_test (Linear_s2: ent_linear_observed = `controls') `Equations' /athrho `athrhos' /lnsigma, vce(robust) missing
*ml init initial, copy
ml maximize
...
*For 2-step, this just involves an main inverse-mills control variable alongside interactions between the IMR and response_after_k_notices indicators for all k larger than 2. The main IMR variable is identified off of the comparison between first (initial) notice and second notice (first reminder). The test of the model is that the interaction terms are jointly zero.
*I prefer FIML.
